...
function get_comments_number_text( $zero = false, $one = false, $more = false, $post_id = 0 ) {
	...
	if ( $number > 1 ) {
		...
		$output = ( false === $zero ) ? __( 'بدون تعليقات' ) : $zero;
	} else { // Must be one.
...
function comment_form_title( $no_reply_text = false, $reply_text = false, $link_to_parent = true ) {
	...
	if ( false === $no_reply_text ) {
		$no_reply_text = __( 'اترك تعليق' );
	}

	if ( false === $reply_text ) {
		/* translators: %s: Author of the comment being replied to. */
		$reply_text = __( 'اترك تعليق to %s' );
	}
...
function comment_form( $args = array(), $post_id = null ) {
	...
	$defaults = array(
		...
		'class_submit'         => 'submit',
		'name_submit'          => 'submit',
		'title_reply'          => __( 'اترك تعليق' ),
		/* translators: %s: Author of the comment being replied to. */
		'title_reply_to'       => __( 'اترك تعليق to %s' ),
